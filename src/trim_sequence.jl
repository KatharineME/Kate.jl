...
function trim_sequence(
    ...
    output_directory_path = splitdir(output_prefix)[1]

    mkpath(output_directory_path)

    print_and_run_cmd(`skewer --threads $n_job -x AGATCGGAAGAGC --compress --output $output_prefix --quiet $_1_fastq_gz_file_path $_2_fastq_gz_file_path`)
